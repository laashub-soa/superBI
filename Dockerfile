...
RUN mkdir /app \
        ...
        && apt-get install -y --no-install-recommends \
            ...
            libpq-dev \
        &&  apt-get install libsasl2-dev libldap2-dev  sasl2-bin libsasl2-modules -y  \
...
RUN useradd --user-group --no-create-home --no-log-init --shell /bin/bash superset \
        ...
        && apt-get install -y --no-install-recommends \
            ...
            default-libmysqlclient-dev \
            libpq-dev \
        &&  apt-get install libsasl2-dev  sasl2-bin libsasl2-modules -y  \
        && rm -rf /var/lib/apt/lists/*
...
USER root
RUN cd /app \
    && pip install --no-cache -r requirements-dev.txt -r requirements-extra.txt -i https://pypi.douban.com/simple
#USER superset
