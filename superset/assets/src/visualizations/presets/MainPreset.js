...
//add custom plugins
import EchartsExample from "../../chartPlugins/echartsExample";
// add Scatter GL
import ScatterGLChartPlugin from '../ScatterGL/ScatterGLChartPlugin';
import Echarts3DLines from "../../chartPlugins/echarts3DLines";
import Odometer from "../../chartPlugins/odometer";
import WordCloud3D from "../../chartPlugins/wordCloud";
import CarouselTable from "../../chartPlugins/carouselTable";
import EchartsPie from "../../chartPlugins/echartsPie";
import EchartsBar from "../../chartPlugins/echartsBar";
import EchartsMultipleLines from "../../chartPlugins/echartsMultipleLines";
import EchartsMapLines from "../../chartPlugins/echartsMapLines";
import EchartsPictorialBar from "../../chartPlugins/echartsPictorialBar";
  ...
  constructor() {
    super({
      ...
      plugins: [
        ...
        new EchartsExample().configure({key: "Echarts"}),
        new ScatterGLChartPlugin().configure({key: "scatter_gl"}),
        new Echarts3DLines().configure({key: "Echarts3DLines"}),
        new Odometer().configure({key: "Odometer"}),
        new WordCloud3D().configure({key: "WordCloud3D"}),
        new CarouselTable().configure({key: "CarouselTable"}),
        new EchartsPie().configure({key: "EchartsPie"}),
        new EchartsBar().configure({key: "EchartsBar"}),
        new EchartsMultipleLines().configure({key: "EchartsMultipleLines"}),
        new EchartsMapLines().configure({key: "EchartsMapLines"}),
        new EchartsPictorialBar().configure({key: "EchartsPictorialBar"})
      ],
    });
  }
}
