...
class BaseViz(object):
    ...
    # get cloumn -> expression dict
    def get_col_expression(self):
        dic={}
        for item in self.datasource.data.get('columns'):
            dic[item['column_name']] = item['expression']
        return dic

    def get_fliters_to_sql(self,query_obj):
        # filter_configs = self.form_data.get("filter_configs")
        filters = self.form_data.get("filters") if self.form_data.get("filters") else []
        filters_sql_str = ""
        extra = query_obj.get("extras")
        if extra :
            filters_sql_str  = extra.get("where")


        logging.info("========================================================================================")
        logging.info(filters_sql_str)
        col_expression = self.get_col_expression()
        # filter_configs_column = []
        # for item in filter_configs:
        #     filter_configs_column.append(item.get('column'))
        for i, item in enumerate(filters):
            # sure the col attributr in databases tabel cloum
            if item.get("col") in self.datasource.column_names:
                col = item.get("col")
                # custom column expression
                if col_expression[col] != None:
                    col = col_expression[col]
                op = item.get("op")
                val = item.get("val")
                val_str = ""
                if isinstance(val,list):
                    ...
                    for index,v in enumerate(val):
                        if self.datasource.columns_types.get(item.get("col")) == 'INTEGER' :
                ...
                if filters_sql_str != "":
                    filters_sql_str += " AND "
                filters_sql_str =  filters_sql_str + col + " " + op + " " + val_str
        # add time range
        # main_dttm_col = self.datasource.data.get("main_dttm_col")
        main_dttm_col = query_obj.get("granularity") if query_obj.get("granularity") else self.datasource.main_dttm_col
        from_dttm = query_obj.get("from_dttm")
        to_dttm = query_obj.get("to_dttm")
        if(from_dttm is not None and to_dttm is not None):
            if filters_sql_str != "":
                filters_sql_str = filters_sql_str + " AND "
            filters_sql_str = filters_sql_str + main_dttm_col + " >= '" + from_dttm.strftime("%Y-%m-%d %H:%M:%S") + "' AND " +  main_dttm_col + " <= '" + to_dttm.strftime("%Y-%m-%d %H:%M:%S") + "'"
        if filters_sql_str == "":
            filters_sql_str = "1=1"
        return filters_sql_str
    ...
    def get_df(
        ...
        # if exist attribute sql,execute this sql
        if self.form_data.get("sql").strip():
            fliters_to_sql = self.get_fliters_to_sql(query_obj)
            logging.info("fliters_to_sql: " + fliters_to_sql)
            self.query = self.form_data.get("sql").strip()
            # if sql str exsit var @filters,replace it by fliters_to_sql
            self.query = self.query.replace("@filters", fliters_to_sql)
            queryList = self.query.split('@split')
            if(len(queryList) > 1):
                list = []
                for i,item in enumerate(queryList):
                    result = self.datasource.database.get_sqla_engine().execute(item)
                    dff = DataFrame(result.fetchall(), columns=result.keys())
                    list.append(dff)
                # df = pd.concat(list,axis=1)
                df = pd.concat(list)
            else:
                logging.info("using custom sql to query")
                logging.info(self.query)
                result = self.datasource.database.get_sqla_engine().execute(self.query)
                # result = self.datasource.database.get_sqla_engine().raw_connection().cursor().execute(self.query,True)
        ...
        # dff = DataFrame(self.datasource.database.get_sqla_engine().execute(self.query).fetchall())
        # df = self.results.df
    ...
    def query_obj(self):
        ...
        if to_dttm is not None:
            to_dttm = to_dttm + timedelta(seconds=-1)
...
class CarouselTableViz(TableViz):
    viz_type = "CarouselTable"
    verbose_name = _("CarouselTable")
...
class EchartsMultipleLines(NVD3Viz):
    """A rich line chart with dual axis"""

    viz_type = "EchartsMultipleLines"
    verbose_name = _("Time Series - Dual Axis Line Chart")
    sort_series = False
    is_timeseries = True
    ...
    def to_series(self, df, df2, classed=""):
        cols = []
        for col in df.columns:
            if col == "":
                cols.append("N/A")
        ...
        series = df.to_dict("series")
        chart_data = []
        line_types = [self.form_data.get("seriestype"), self.form_data.get("seriestype_2")]
        metrics = [self.form_data.get("metric"), self.form_data.get("metric_2")]
        # get a object type => metric
        types_metric = {}
        bar_index = 0
        for i,type in enumerate(line_types):
            if type == 'bar' :
                bar_index = i
            types_metric[type] = metrics[i]
        for i, m in enumerate(metrics):
            m = utils.get_metric_name(m)
            ys = series[m]
            if df[m].dtype.kind not in "biufc":
            ...
            if ('groupby' in df2.columns and i != bar_index) or  'groupby' not in df2.columns:
                chart_data.append(d)
        # modify groupby data
        if 'groupby' in df2.columns:
            groupby = df2.groupby('groupby')
            groupby_list = []
            for name,group in groupby:
                group = group.set_index([df.index.name])
                group_series = group.to_dict("series")
                ys = group_series[types_metric['bar']]
                d = {
                    "key": name,
                    "classed": classed,
                    "values": [
                        {"x": ds, "y": ys[ds] if ds in ys else None} for ds in group.index
                    ],
                    "groupby":True,
                    "yAxis": bar_index + 1,
                    "type": "bar",
    ...
    def get_data(self, df):
        ...
        metric = utils.get_metric_name(fd.get("metric"))
        metric_2 = utils.get_metric_name(fd.get("metric_2"))
        dff = df.pivot_table(index=DTTM_ALIAS, values=[metric, metric_2])

        chart_data = self.to_series(dff,df)
        return chart_data
...
class EchartsPieViz(DistributionPieViz):
    viz_type = "EchartsPie"
    verbose_name = _("Ehcarts Pie Chart")
...
class EchartsBarViz(DistributionBarViz):
    """echarts bar chart"""
    viz_type = "EchartsBar"
    sort_series = True
    verbose_name = _("Echarts Bar Chart")


class EchartsPictorialBarViz(DistributionBarViz):
    """echarts pictorial bar chart"""
    viz_type = "EchartsPictorialBar"
    sort_series = True
    verbose_name = _("Echarts Pictorial Bar Chart")
...
class Echarts3DLines(BaseDeckGLViz):

    """deck.gl's ScatterLayer"""

    viz_type = "Echarts3DLines"
    verbose_name = _("Deck.gl - Scatter plot")
    spatial_control_keys = ["start_point", "end_point"]
    is_timeseries = True
...
class EchartsMapLines(BaseDeckGLViz):
    
    """deck.gl's ScatterLayer"""

    viz_type = "EchartsMapLines"
    # verbose_name = _("Deck.gl - Scatter plot")
    spatial_control_keys = ["start_point", "end_point"]
    is_timeseries = True
...
class ScatterGLViz(BaseDeckGLViz):
    
    viz_type = "scatter_gl"
    verbose_name = _("Scatter GL")
    spatial_control_keys = ["spatial"]

    def get_properties(self, d):
        return {d.get("spatial")[0], d.get("spatial")[1]}
